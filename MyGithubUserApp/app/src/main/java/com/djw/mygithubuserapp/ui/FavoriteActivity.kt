...
class FavoriteActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        detailViewModel.getFavorite().observe(this) { user ->
            binding.progressBar.visibility = View.GONE
            val listUser = user.map {
                ItemsItem(it.avatar_url, it.username)
            }

            val adapter = ListUserAdapter(listUser as ArrayList<ItemsItem> /* = java.util.ArrayList<com.djw.mygithubuserapp.database.remote.response.ItemsItem> */)
